...
{
	uint8_t charBuf[20]; 
	sprintf(charBuf,"%x",val);
	uint32_t temp = atoi(charBuf);
	val = temp;
...
/* 发送头 */
static int sendHeader( ymodem_t *ym, const char *filename, int retry_cnt, uint32_t filesize){
	int bdata;
	int filename_len;
	int packet_size;
	int ret;
	...
	/* copy file size to buffer*/
	uint32_t res = ym_writeU32(filesize, ym->buffer+filename_len);

	arraySet( ym->buffer+filename_len+res, 0, YM_PACKET_SIZE_1K-filename_len-res );
 ...
 * @param filenane 
 */
int ymodem_startTransmit( ymodem_t *ym, const char *filename, int retry_cnt, uint32_t filesize){
	int bdata;
	int filename_len;
	int packet_size;
	int ret;
	...
	ret = sendHeader( ym, filename, retry_cnt, filesize);
	if( ret != YM_SUCCESS ){
